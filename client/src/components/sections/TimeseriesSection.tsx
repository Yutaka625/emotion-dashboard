...
import { Plus, Trash2, ChevronDown, ChevronUp, Tag, Download, Target, Wand2, Activity } from 'lucide-react';
...
export default function TimeseriesSection({ data }: Props) {
  ...
  const [smoothingWindow, setSmoothingWindow] = useState(15);  // SMA: ウィンドウサイズ（フレーム数）
  const [smoothingAlpha, setSmoothingAlpha] = useState(0.25); // EMA: スムージング係数 (0=強平滑, 1=原波形)
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <span style={{ fontFamily: 'Roboto Mono, monospace', fontSize: '0.82rem', fontWeight: 700, color: 'oklch(0.88 0.005 250)' }}>
                ...
                <span style={{ fontSize: '0.65rem', color: 'oklch(0.58 0.015 255)', marginLeft: '6px' }}>
                  (約 {(smoothingWindow / data.meta.fps_avg).toFixed(1)}秒)
                </span>
              </span>
            </div>
            <input
