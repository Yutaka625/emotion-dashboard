...
- [ ] **マルチ FaceID の拡張**
  - **FaceID のノイズフィルタ＋品質可視化**（旧「10+ ドロップダウン表示」から方針変更）
    - 背景: FaceID が多い＝多人数ではなく、検出の不安定さ（同一人物の再採番・誤検出）が主因。
      数フレームしかない FaceID も生じる。個別選択UIより、ノイズ除去とデータ品質の明示が本質。
    - 少フレームの FaceID（例: 全フレームの 5% 未満 or 3秒未満。閾値は実データ分布で確定）を
      「副次的（noise）」として既定で解析対象から除外する
    - 「FaceID を N 個検出。うち M 個は少数フレームのため除外（検出が不安定な可能性）」を
      バナー/注記で明示する（`csvAnalyzer` のグルーピング段階＋ FaceIDSelector 付近）
    - 除外により「全員（allCombined）」集計の質も改善する
    - 任意: 「微小な ID も表示」トグルで透明性を確保（必要な人は全件参照可）
    - フィルタ後はたいてい少数に収まるため、選択UIは現行チップで足りる見込み。
      正当な多人数（安定検出）が実在した場合のみ、改めてチップ折り返し/軽いドロップダウンを検討
