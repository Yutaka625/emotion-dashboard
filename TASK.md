...
## 🧪 検証タスク（無償公開前の確認用）
- [ ] **トップ画面（D&D画面）に利用規約・プライバシーポリシーへのリンク／ボタンを表示**
  - 無償公開前の確認用として、`KSDV_terms.html` への導線をトップ画面に配置する
  - 配置位置：使い方ガイド（`KSDV_User-Guide.html`）リンクの**すぐ下**（`DropZone.tsx` のガイドリンク付近）
  - ※ 本格運用時は「同意フロー（クリックラップ）」タスクに発展させる想定（下記「機能拡張」参照）
- [ ] **解析機能テスト用のサンプル感情ログファイルを作成**（2-a）
  - 心sensor 準拠（Affectiva 形式）の感情ログCSVを用意する
  - 基本要件：**45秒以上**の解析データ（動態指標 AR1/MSSD やベースライン補正が意味を持つ長さ）
  - KSDV の各種解析機能（時系列・ベースライン補正・変化点・象限分類・A/B比較・統計検定・マルチFaceID 等）を
    一通り検証できるよう、機能ごとに適したパターンのファイルを用意する
- [ ] **テスト用サンプルを読み込むボタンをトップ画面に配置**（2-b）
  - 機能テストごとにボタンを用意し、押すと対応するサンプルファイルが読み込まれ、解析結果画面に遷移する
  - 配置：トップ画面（D&D画面）。どの機能を検証するためのサンプルか分かるラベルを付ける
  - ※ 2-a のサンプルファイル作成が前提
