...
## 🆕 2026-06-18 追加（CX/体験価値タブ: 設計検討）
- [ ] **CX/体験価値タブの設計検討（AIインサイト関連と併せて深掘り）**: 当初の「マーケティング」タブ案は広告効果測定に寄りすぎるため、日本の現場ニーズでは `CX/体験価値` として再定義する方向で継続検討する。対象ユースケースはイベント来場体験、店舗導線、ホテル/受付などの接客体験、空間・ブランド接点の評価。
  - **UXリサーチとの差分**: UXリサーチは「タスク・UIの使いづらさ」、CX/体験価値は「接点ごとの体験品質・印象・リスク」を扱う。既存指標の焼き直しにならないよう、新しい感情指標を増やすより `接点別評価・比較・改善仮説` を主役にする。
  - **心sensorログだけで導ける範囲**: 接点別の反応パターン、ピーク、差分、比較まではAIなしで定量化できる。`期待形成`、`安心感`、`納得感`、`ブランド印象への影響` は直接測定ではなく、engagement / valence / attention / joy / confusion などを用いた代理指標として扱う。
  - **AIインサイトとの関係**: v1実装にAIは必須ではない。将来的にAIインサイト機能と連携し、イベント内容、接客メモ、導線説明、アンケートなど外部文脈を含めた要約・改善仮説生成に使う。
  - **次回検討ポイント**: タブ名、接点定義の入力方法、既存イベント注釈との接続、担当者比較の倫理・プライバシー表現、Bizティア/AIインサイトとの境界を整理する。
