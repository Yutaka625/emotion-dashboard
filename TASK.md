...
## 🟢 低優先度（後回し可）
- [ ] **ユーザーガイドの画面解説を追記（他機能とまとめて）**: 学術機能で追加した「感情分布タブの記述統計テーブルの n（有効フレーム数）列」「学術タブの変化点一覧テーブル（delta閾値スライダー・列ソート）」をガイドの画面解説に追記する。※CSV列定義は更新済み・画面解説のみ残。今後の機能追加分とまとめて一括対応（`KSDV_User-Guide.html` ＋ `client/public/` 同期）
- [ ] **マルチFaceID の Emotion Overlay に実時間表示を追加**: 現状は各顔の登場区間を0〜100%に正規化した「進行率」グラフのみ。比較分析タブの TIMELINE OVERLAY と同様に、**実時間（秒）／正規化（進行率）の切替トグル**を追加する（比較分析タブには実装済み・未対応はマルチFaceID比較オーバーレイのみ）。実装は比較分析タブの時間軸モードトグルを範とする。注意: 顔ごとに登場時刻・長さが異なるため、実時間表示では映像内の同じ瞬間には揃わない（その旨の補足を添える）
