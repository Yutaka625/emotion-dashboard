...
- [ ] **マルチFaceID の Emotion Overlay に実時間表示トグルを追加** 〔難易:中(既存流用)／影響:小(1セクション)／UX:中〕: 現状は各顔の登場区間を0〜100%に正規化した「進行率」グラフのみ。比較分析タブの TIMELINE OVERLAY と同様に**実時間（秒）／正規化（進行率）の切替トグル**を追加（実装は比較分析タブの時間軸モードトグルを範とする）。注意: 顔ごとに登場時刻・長さが異なるため、実時間表示では映像内の同じ瞬間には揃わない（その旨の補足を添える）
