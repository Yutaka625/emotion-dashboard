...
## 2026-06-20 セッション（ユーザーガイド: 活用シナリオ・ユースケース追加）
- **ユーザーガイドに「活用シナリオ」を追加（feature/user-guide-use-cases）**: ナビゲーションに「活用シナリオ」を新設し、`ユースケース` と `学術活用ガイド` を配置。ユースケース章は、操作方法・タブ別ガイドを読んだ後に理解しやすいよう、`比較分析` の後・参考資料の前へ移動
- **シーン別ユースケースを追加**: 研究者／UXリサーチャー／マーケターの3カードと、各詳細ユースケース（目的・実験計画・KSDVで見る画面・読み取る指標・レポート記述例）を追加。表情データは単独で断定せず、行動ログ・発話・アンケート等と照合して解釈する前提を明記
- **学術活用ガイドの位置づけを整理**: `学術活用ガイド` を参考資料から活用シナリオ配下へ移し、`指標クイックリファレンス` は参考資料に単独配置。章順を `ユースケース` → `学術活用ガイド` → `指標クイックリファレンス` に整理
- **指標クイックリファレンスを実データ仕様に修正**: 見出しを「単位/範囲」→「範囲」に変更。感情スコア・Engagement・Attentionの `%` 表記を削除。Valenceを `-100〜+100（0=中立）` とし、マイナス=ネガティブ／プラス=ポジティブに修正。Attentionは「注目度（顔の向き）。正面を向いていると100。視線は影響しない」と明記
- **資料同期・検証**: `KSDV_User-Guide.html` と `client/public/KSDV_User-Guide.html` を同期（ハッシュ一致）。`git diff --check` 通過。アプリ本体コード・CSV仕様・公開APIの変更はなし
