...
## 🟢 低優先度（後回し可）
- [ ] **別環境のClaude Codeメモリへ同期**: このリポジトリで作業した環境のClaude メモリ（IQR外れ値不採用・5資料チェックリスト・フレーム擬似反復の3件）を、別の作業環境のメモリにも追記する。内容は `docs/claude-memory-handoff.md` に転記済み。別環境ではそのファイルをClaudeに読ませて「メモリへ追記して」と依頼する（既存同名は統合）。※メモリは各PCローカルでGit同期されないための手動同期タスク
