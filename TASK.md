...
## 🟢 低優先度（後回し可）
- [ ] **マルチFaceIDオーバーレイに実時間（秒）表示トグルを追加**（任意）: 現状は各顔の登場区間を0〜100%に正規化した「進行率」表示（意味の説明はUI・ガイドに追加済み）。秒表示も選べるようにする案。ただし顔ごとに登場時刻・長さが異なり、秒では映像内の同じ瞬間に揃わない点に注意（比較分析タブの TIMELINE OVERLAY には実装済み。未対応はマルチFaceID比較オーバーレイのみ）
