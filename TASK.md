...
- [ ] **マルチFaceIDの時間正規化**: X軸の「%」は何を意味するか？実時間（秒）に変換できるか？
- [ ] **比較分析タブのサブタブ表示**: 比較分析時に別タブに表示されているデータはA/Bどちらのデータか？表示切替は可能か？
- [ ] **特殊指標タブ: Engagement × Valence 散布図**: 散布図は何を意味するか？凡例に「色=支配的感情」とあるがN/Aになり続ける原因は？
- [ ] **特殊指標タブ: 特殊指標レベル分布が常に空**: 何も表示されない原因の調査
- [ ] **特殊指標タブ: 集計数値（平均値・最大値・中央値・標準偏差）**: どの指標に関する数値か分かりにくい。ラベル改善を検討
- [ ] **特殊指標タブ: 特殊指標と各指標の相関係数**: 何と何の相関を表しているか説明が不足。UI上の説明文追加を検討
- [ ] **感情分布タブ: 感情動態指標の「慣性」グラフ非表示**: 変動性・不安定性はグラフ表示されるが慣性だけ表示されない原因の調査
- [ ] **学術分析タブ: 感情変動性の根拠**: 「FearとEngagementの変動性が高い」という記述の根拠（計算式・表示条件）の確認
- [ ] **感情分布タブ: 感情出現率（閾値超過の割合）**: 「閾値」の値は何か？設定変更は可能か？
